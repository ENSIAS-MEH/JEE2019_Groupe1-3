...
public class TerrainDaoImpl implements TerrainDao {
	...
	public Terrain getTerrain(long id_terrain) throws DAOException {
		Connection connexion = null;
		PreparedStatement preparedStatement = null;
		ResultSet resultSet = null;
		...
		try {
			...
			while (resultSet.next()) {
				//terrain = new Terrain();
				terrain.setId_terrain(resultSet.getLong("id_terrain"));
				...
				terrain.setAddress(resultSet.getString("address"));
				terrain.setVille(resultSet.getString("ville"));
				terrain.setCodePostale(resultSet.getString("codePostale"));
				terrain.setInfoClub(resultSet.getString("infoClub"));
				terrain.setUrl(resultSet.getString("url"));
				terrain.setId_propriétaire(resultSet.getLong("id_proprietaire"));
