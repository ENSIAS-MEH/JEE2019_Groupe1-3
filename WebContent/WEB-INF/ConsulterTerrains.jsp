...
<body>
	...
	<div class="container">
		<h1 class="text-center">Mes Terrains</h1>
		</br>
		...
		<div class="row">
			<c:forEach items="${listeTerrains}" var="terrain">
				...
				<form class="modal-popup-content animate" method ="get">
					<div class="imgcontainer">
						<span
							onclick="document.getElementById('modal-wrapper${terrain.id_terrain}').style.display='none'"
							class="close" title="Close PopUp">&times;
						</span> 
						<img src="terrainsImages/${terrain.image}" alt="Avatar" class="avatar">
						<h1 style="text-align: center"><!--${terrain.image} --></h1>
					</div>
					<div class="container">
						<h4 class="text-center">${terrain.description }</h4>
						<h5 class="text-center">Prix: ${terrain.prix } MAD/heurer</h5>
						<br/>
						<h6 class="text-center">Address : ${terrain.address }</h6>
						<h6 class="text-center">Ville : ${terrain.ville }</h6>
						<h5 class="text-center">Club : ${terrain.infoClub}</h5>
	            		<input type="hidden" name="id_terrain" id="id_terrain" value="${terrain.id_terrain}" />	
	            	    <input type="hidden" name="id_proprietaire" id="id_proprietaire" value="${terrain.id_propriétaire}" />							
	            		<br/>
						<button type="submit" formaction="ModificationTerrain">Modifier</button>
						<br/>
						<br/>
						<button type="submit" formaction="suppressionTerrain">Supprimer</button>
					</div>
				</form>
			</div>
